...
player_actions = {"door": None, "playing": True, "choice": None, "won": False}
inventory = {}
...
while player_actions["playing"] == True:
    ...
    # calling methods depending on which door was chosen
    if player_actions["door"] == "left":
        left_door()
    elif player_actions["door"] == "middle":
        middle_door()
...
def write_to_file():
    path = pathlib.Path("/home/junaid/Desktop/Python/CLI_rpg")
    x = datetime.datetime.now()
    
    if player_actions["won"] == True:
        string = f"{name} beat the dragon with the {inventory['sword']} - {x.hour}:{x.minute} {x.day}:{x.month}:{x.year}\n"
    elif player_actions["won"] == False and "sword" in inventory:
        string = f"{name} lost to the dragon with the {inventory['sword']} - {x.hour}:{x.minute} {x.day}:{x.month}:{x.year}\n"
    else:
        string = f"{name} lacked a weapon and got killed by the dragon - {x.hour}:{x.minute} {x.day}:{x.month}:{x.year}\n"

    with open(path.joinpath("leaderboard.txt"), "a") as file_out:
